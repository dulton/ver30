...
class IPC_MediaDataServer2
{
public:
    IPC_MediaDataServer2(void);
    ~IPC_MediaDataServer2(void);
...
private:
    ...
    GMI_RESULT     NotifyOfClient      ();

private:
    ApplicationPacket                                                                                     m_MediaDataServerRequestPacket;
    ApplicationPacket                                                                                     m_MediaDataServerReplyPacket;
    ...
    std::map< uint16_t, ReferrencePtr<IpcMediaDataUDPSession,DefaultObjectDeleter,MultipleThreadModel> >  m_ClientSessions;
    std::map< uint32_t, ReferrencePtr<IpcMediaDataUDPSession,DefaultObjectDeleter,MultipleThreadModel> >  m_ClientApplications;
    uint32_t                                                                                              m_MediaType;
    uint32_t                                                                                              m_MediaId;
    long_t                                                                                                m_ShareMemoryKey;
    GMI_ShareMemory                                                                                       m_ShareMemory;
    void_t                                                                                                *m_MemoryPool;
    GMI_Mutex                                                                                             m_DataBlockInfoLock;
    struct DataBlockInfo
    {
        uint32_t     s_Offset;
        uint32_t     s_Length;
    };
    std::list<struct DataBlockInfo>                                                                       m_DataBlockInfoList;
