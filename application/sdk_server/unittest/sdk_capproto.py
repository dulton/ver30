...
import xunit.extlib.sdkproto.capproto as CLSCapProto
...
def __GetCapProto(ssock,catagory=0):
	capproto = ssock.GetCapProto(catagory)
	print('%s'%(repr(capproto)))
	return 
	
def GetCapProtoUserPassword(host,port,username,password,catagory=0):
	ssock = xunit.extlib.sdksock.SdkCapProtoSock(host,port)
	sesid = ssock.LoginUserPass(username,password)
	return __GetCapProto(ssock,catagory)
...
def VerifyOptions(opts,args):
	...
	Usage(args,3,'please specify --getcfg or --username')
...
def main():
	...
	args.add_option('-g','--getcfg',action='store',type='int',nargs=1,dest='getcfg',default=None,help='to get capabilities')
	...
	if options.session :
		...
		GetCapProtoUserPassword(options.host,options.port,options.user,options.password,options.getcfg)
	return
