...
SysSetUserInfoCommandExecutor::SysSetUserInfoCommandExecutor(void)
    : SysBaseCommandExecutor(SYSCODE_SET_USERINFO_REQ, CT_ShortTask )
{
}
...
GMI_RESULT  SysSetUserInfoCommandExecutor::Create(ReferrencePtr<BasePacket>& Packet, SafePtr<BaseCommandExecutor>& CommandExecutor )
{
    ...
    SetUserInfoCommandExecutor->m_Session = Packet->GetSession();
    SetUserInfoCommandExecutor->m_Reply   = Packet->Clone();
    GMI_RESULT Result = SetUserInfoCommandExecutor->SetParameter(m_SystemServiceManager, m_Argument, m_ArgumentSize);
    if (FAILED(Result))
    {
        return Result;
    }

    CommandExecutor = SetUserInfoCommandExecutor;

    return GMI_SUCCESS;
}


GMI_RESULT  SysSetUserInfoCommandExecutor::Execute()
{
    SYS_INFO("%s:%s in..........\n", __FILE__, __func__);
    ...
    if (ReqAttrCnt > 0)
    {
        ...
        for (i = 0; i < ReqAttrCnt; i++)
        {
            ...
            if (0 == strlen(SysUserInfoPtr->s_UserName))
            {
                SYS_ERROR("username is null\n");
                DEBUG_LOG(g_DefaultLogClient, e_DebugLogLevel_Exception, "username is null\n");
                break;
            }
        ...
        if (i >= ReqAttrCnt)
        {
            ...
            for (i = 0; i < ReqAttrCnt; i++)
            {
                ...
                Result = m_SystemServiceManager->SvrSetUser(SysUserInfoPtr);
                if (FAILED(Result))
                {
...
GMI_RESULT  SysSetUserInfoCommandExecutor::Reply()
{
