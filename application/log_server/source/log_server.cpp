...
GMI_RESULT GetLogServerPipeName( char_t *PipeName, size_t *BufferLength )
{
    ...
    if ( FAILED( Result ) )
    {
        printf( "GetLogServerPipeName, SysInfoOpen, Result=%x \n", (uint32_t) Result );
        return Result;
    }

    printf( "log server, GetLogServerPipeName, Default_PipeName=%s \n", LOG_SERVER_DEFAULT_PIPE_NAME );

    Result = SysInfoRead(Handle, LOG_SERVER_CONFIG_PATH, LOG_SERVER_CONFIG_SERVER_PIPE_NAME, LOG_SERVER_DEFAULT_PIPE_NAME, PipeName );
    if ( FAILED( Result ) )
    {
        printf( "GetLogServerPipeName, SysInfoRead, Result=%x \n", (uint32_t) Result );
        return Result;
    }
    ...
    if ( FAILED( Result ) )
    {
        printf( "GetLogServerPipeName, SysInfoClose, Result=%x \n", (uint32_t) Result );
        return Result;
    }
...
GMI_RESULT GetLogServerPipeMutexId( uint32_t *MutexId )
{
    ...
    Result = SysInfoRead(Handle, LOG_SERVER_CONFIG_PATH, LOG_SERVER_CONFIG_SERVER_MUTEX_ID, LOG_SERVER_DEFAULT_PIPE_MUTEX_ID, (int32_t *) MutexId );
    if ( FAILED( Result ) )
    {
        return Result;
    }
