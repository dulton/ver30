...
void LogModule::Print(LogLevel Level, const char * File, const char * Func, unsigned int Line, const char * Fmt, ...)
{
    ...
    snprintf(FmtBuf + PREFIX_SIZE, sizeof(FmtBuf) - PREFIX_SIZE - SUFFIX_SIZE, "%4d-%02d-%02d %02d:%02d:%02d %9s %25s line %05d: %s\n",
...
void LogModule::DumpBuffer(LogLevel Level, const char * File, const char * Func, unsigned int Line, const char * Variable, const void * Buf, unsigned int BufLength)
{
    // Check if we need to precess this log
    if ((Level > m_LogWriteLevel && Level > m_LogDispLevel) || Level == eNone)
    {
    ...
    Print(Level, File, Func, Line, "%s =", Variable);
