...
#endif
{
    ...
    Result = GetMediaCenterServerLogConfig( &ModuleId, ModuleName, ModulePipeName, &ModulePipeMutexId, PeerPipeName, &PeerPipeMutexId, ServerPipeName, &ServerPipeMutexId, &ServerDebugLogLevel );
    if ( FAILED( Result ) )
    {
...
GMI_RESULT GetMediaCenterServerLogConfig( uint32_t *ModuleId, char_t *ModuleName, char_t *ModulePipeName, long_t *ModulePipeMutexId, char_t *PeerPipeName, long_t *PeerPipeMutexId, char_t *ServerPipeName, long_t *ServerPipeMutexId, uint32_t *DebugLogLevel )
{
    *ModuleId = GMI_LOG_MODULE_MEDIA_CENTER_SERVER_ID;
    ...
    Result = SysInfoRead(Handle, MEDIA_CENTER_SERVER_CONFIG_PATH, MEDIA_CENTER_SERVER_CONFIG_LOG_CLIENT_PIPE_NAME, LOG_MEDIA_CENTER_DEFAULT_CLIENT_PIPE_NAME, ModulePipeName );
    if ( FAILED( Result ) )
    {
        return Result;
    }
    ...
    Result = SysInfoRead(Handle, MEDIA_CENTER_SERVER_CONFIG_PATH, MEDIA_CENTER_SERVER_CONFIG_LOG_CLIENT_PIPE_MUTEX_ID, LOG_MEDIA_CENTER_DEFAULT_CLIENT_PIPE_MUTEX_ID, (int32_t *) ModulePipeMutexId );
    if ( FAILED( Result ) )
    {
        return Result;
    }
    ...
    Result = SysInfoRead(Handle, MEDIA_CENTER_SERVER_CONFIG_PATH, MEDIA_CENTER_SERVER_CONFIG_LOG_PEER_PIPE_NAME, LOG_MEDIA_CENTER_DEFAULT_PEER_PIPE_NAME, PeerPipeName );
    if ( FAILED( Result ) )
    {
        return Result;
    }
    ...
    Result = SysInfoRead(Handle, MEDIA_CENTER_SERVER_CONFIG_PATH, MEDIA_CENTER_SERVER_CONFIG_LOG_PEER_PIPE_MUTEX_ID, LOG_MEDIA_CENTER_DEFAULT_PEER_PIPE_MUTEX_ID, (int32_t *) PeerPipeMutexId );
    if ( FAILED( Result ) )
    {
        return Result;
    }
    ...
    Result = SysInfoRead(Handle, MEDIA_CENTER_SERVER_CONFIG_PATH, MEDIA_CENTER_SERVER_CONFIG_LOG_SERVER_PIPE_NAME, LOG_SERVER_DEFAULT_PIPE_NAME, ServerPipeName );
    if ( FAILED( Result ) )
    {
        return Result;
    }
    ...
    Result = SysInfoRead(Handle, MEDIA_CENTER_SERVER_CONFIG_PATH, MEDIA_CENTER_SERVER_CONFIG_LOG_SERVER_PIPE_MUTEX_ID, LOG_SERVER_DEFAULT_PIPE_MUTEX_ID, (int32_t *) ServerPipeMutexId );
    if ( FAILED( Result ) )
    {
        return Result;
    }
    ...
    Result = SysInfoRead(Handle, MEDIA_CENTER_SERVER_CONFIG_PATH, MEDIA_CENTER_SERVER_CONFIG_DEBUG_LOG_LEVEL, GMI_LOG_MODULE_MEDIA_CENTER_SERVER_DEBUG_LOG_LEVEL, (int32_t *) DebugLogLevel );
    if ( FAILED( Result ) )
    {
        return Result;
    }
...
GMI_RESULT GetHeartbeatInterval( uint32_t *Interval )
{
    ...
    Result = SysInfoRead(Handle, MEDIA_CENTER_SERVER_CONFIG_PATH, MEDIA_CENTER_SERVER_CONFIG_HEARTBEAT_INTERVAL, GMI_MEDIA_CENTER_SERVER_HEARTBEAT_INTERVAL, (int32_t *) Interval );
    if ( FAILED( Result ) )
    {
        return Result;
    }
...
GMI_RESULT GetMediaCenterServerPort( uint16_t *Port )
{
    ...
    if ( FAILED( Result ) )
    {
        printf( "GetMediaCenterServerPort, SysInfoOpen, Result=%x \n", (uint32_t) Result );
        return Result;
    }

    printf( "GetMediaCenterServerPort, Default_Port=%d \n", GMI_MEDIA_CENTER_SERVER_COMMAND_PORT );

    Result = SysInfoRead(Handle, MEDIA_CENTER_SERVER_CONFIG_PATH, MEDIA_CENTER_SERVER_CONFIG_SERVER_PORT, GMI_MEDIA_CENTER_SERVER_COMMAND_PORT, &ServerPort );
    if ( FAILED( Result ) )
    {
        printf( "GetMediaCenterServerPort, SysInfoRead, Result=%x \n", (uint32_t) Result );
        return Result;
    }
    ...
    if ( FAILED( Result ) )
    {
        printf( "GetMediaCenterServerPort, SysInfoClose, Result=%x \n", (uint32_t) Result );
        return Result;
    }
