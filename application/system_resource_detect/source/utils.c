...
long percentages(int cnt, int *out, register long *new_value, register long *old, long *diffs)
{
    register int i;
    ...
    for (i = 0; i < cnt; i++)
    {
        if ((change = *new_value - *old) < 0)
        {
            /* this only happens when the counter wraps */
            change = (int)
                     ((unsigned long)*new_value-(unsigned long)*old);
        }
