...
GMI_RESULT SystemServiceManager::SvrPtzControl(SysPkgPtzCtrl *PtzCtrl )
{
	...
	if (m_SupportPtz)
	{
				...
				|| SYS_PTZCMD_RIGHTDOWN == PtzCtrlTmp.s_PtzCmd)
		{
			...
			case SYS_PTZCMD_LEFT:
			...
			#ifdef TEST_STORAGE
				memset(&RecordFileQueryPtr, 0, sizeof(RecordFileQueryPtr));
				...
				RecordFileQueryResPtr = (RecordFileQueryResOut **)malloc(sizeof(RecordFileQueryResOut *)*MAX_NUM_QUERY_RECORD);
				...
				{
					RecordFileQueryResPtr[i_k] = (RecordFileQueryResOut *)malloc(sizeof(RecordFileQueryResOut));
			...
			QueryLoop:
									 ...
									 &QueryResTotalNum, &QueryResCurNum)))
				{
					...
					DEBUG_LOG(g_DefaultLogClient, e_DebugLogLevel_Exception, "#####GMI_RecordFileQuery, Result = 0x%lx\n", Result);
				}
				else
				{
					...
					{
						SYS_ERROR("RecFile %d:[%s]	[%d]  [%d-%d]\n", i_k, RecordFileQueryResPtr[i_k]->s_RecFileName, 
							RecordFileQueryResPtr[i_k]->s_RecFileSize, RecordFileQueryResPtr[i_k]->s_RecFileTime[0], RecordFileQueryResPtr[i_k]->s_RecFileTime[1]);
					}

					
					if(QueryResCurNum < QueryResTotalNum)
					{
						SYS_ERROR("###GMI_RecordFileQuery %d < %d\n", QueryResCurNum, QueryResTotalNum);
						goto QueryLoop;
					}

					
					RecordDownReplayQuery.s_RecQueryType = 0;
					memset(RecordDownReplayQuery.RecDownReplayInfo.s_RecFileName, 0, sizeof(RecordDownReplayQuery.RecDownReplayInfo.s_RecFileName));
					memcpy(RecordDownReplayQuery.RecDownReplayInfo.s_RecFileName, RecordFileQueryResPtr[0]->s_RecFileName, strlen(RecordFileQueryResPtr[0]->s_RecFileName));
					SYS_ERROR("RecordFileQueryResPtr[0]->s_RecFileName=%s, RecordDownReplayQuery.RecDownReplayInfo.s_RecFileName=%s.\n", 
						RecordFileQueryResPtr[0]->s_RecFileName, RecordDownReplayQuery.RecDownReplayInfo.s_RecFileName);
				}
			...
			case SYS_PTZCMD_RIGHT:
				...
				PT_CtlCmd.s_CmdParam[0] = FloatToInt((float_t)PtzCtrlTmp.s_Param[0]*100/255);
			#ifdef TEST_STORAGE
				RecordDownReplayQueryResPtr = (RecordDownReplayQueryResOut**)malloc(sizeof(RecordDownReplayQueryResOut *)*QueryResArraySize2);
				...
				{
					RecordDownReplayQueryResPtr[i_k] = (RecordDownReplayQueryResOut *)malloc(sizeof(RecordDownReplayQueryResOut));
					if(NULL == RecordDownReplayQueryResPtr[i_k])
					{
						SYS_ERROR("RecordDownReplayQueryResPtr[%d] malloc error.\n", i_k);
						break;
					}
					memset(RecordDownReplayQueryResPtr[i_k], 0, sizeof(RecordDownReplayQueryResOut));
				}
				SYS_ERROR("RecordDownReplayQuery s_RecFileName=%s.\n", RecordDownReplayQuery.RecDownReplayInfo.s_RecFileName);
				if(FAILED(GMI_RecordDownReplayQuery(&RecordDownReplayQuery, RecordDownReplayQueryResPtr, QueryResArraySize2)))
				{
				...
				else
				{
					...
					for(i_k=0;i_k<QueryResArraySize2; i_k++)
					{
						...
						SYS_ERROR("s_StreamType=%d[0-ps,1-es]\n", RecordDownReplayQueryResPtr[i_k]->s_StreamType);
						...
						SYS_ERROR("s_VideoFrame=%d\n", RecordDownReplayQueryResPtr[i_k]->s_VideoFrame);
						SYS_ERROR("s_AudioFrame=%d\n", RecordDownReplayQueryResPtr[i_k]->s_AudioFrame);
						SYS_ERROR("s_VideoWide=%d\n", RecordDownReplayQueryResPtr[i_k]->s_VideoWide);
						SYS_ERROR("s_VideoHeight=%d\n", RecordDownReplayQueryResPtr[i_k]->s_VideoHeight);
						SYS_ERROR("s_RecFileName=%s\n", RecordDownReplayQueryResPtr[i_k]->s_RecFileName);
						SYS_ERROR("s_RecFileOffset=%d\n", RecordDownReplayQueryResPtr[i_k]->s_RecFileOffset);
						SYS_ERROR("s_RecFileSize=%d\n", RecordDownReplayQueryResPtr[i_k]->s_RecFileSize);
						SYS_ERROR("s_RecFileReplayNum=%d\n", RecordDownReplayQueryResPtr[i_k]->s_RecFileReplayNum);
						SYS_ERROR("s_RecFileCurNo=%d\n", RecordDownReplayQueryResPtr[i_k]->s_RecFileCurNo);
						SYS_ERROR("s_CurFileStartTime=%d\n", RecordDownReplayQueryResPtr[i_k]->s_CurFileStartTime);
						SYS_ERROR("s_CurFileEndTime=%d\n", RecordDownReplayQueryResPtr[i_k]->s_CurFileEndTime);
						SYS_ERROR("\n");
					}
					SYS_ERROR("***************************************\n");
				}
			...
			case SYS_PTZCMD_UP:
				...
				if(FAILED(GMI_StorageDeviceUninit(&StorageUninitParam)))
				{
					...
					DEBUG_LOG(g_DefaultLogClient, e_DebugLogLevel_Exception, "#####GMI_StorageDeviceUninit, Result = 0x%lx\n", Result);
				}
			#endif
				break;
			case SYS_PTZCMD_DOWN:
				...
				if(FAILED(GMI_RecordScheduleConfig(&RecordScheduleConfig)))
				{
					...
					DEBUG_LOG(g_DefaultLogClient, e_DebugLogLevel_Exception, "#####GMI_RecordScheduleConfig111, Result = 0x%lx\n", Result);
				}
			#endif
				break;
			case SYS_PTZCMD_LEFTUP:
				...
				if(FAILED(GMI_StorageDeviceFormat(&StorageFormatParam)))
				{
					...
					DEBUG_LOG(g_DefaultLogClient, e_DebugLogLevel_Exception, "#####GMI_StorageDeviceFormat, Result = 0x%lx\n", Result);
				}
			#endif
			...
			case SYS_PTZCMD_LEFTDOWN:
				...
				if(FAILED(GMI_StorageDeviceInit(&StorageInitParam)))
				{
					...
					DEBUG_LOG(g_DefaultLogClient, e_DebugLogLevel_Exception, "#####GMI_StorageDeviceInit, Result = 0x%lx\n", Result);
				}
			#endif
				break;
			case SYS_PTZCMD_RIGHTUP:
				...
				PT_CtlCmd.s_CmdParam[1] = FloatToInt((float_t)PtzCtrlTmp.s_Param[0]*100/255);
			#ifdef TEST_STORAGE
				RecordCtrlIn RecordCtrl;
				memset(&RecordCtrl, 0, sizeof(RecordCtrlIn));
				RecordCtrl.s_RecCtrlFlag = 1;
				RecordCtrl.s_EncodeType = 1;
				RecordCtrl.s_VideoFrame = 25;
				RecordCtrl.s_VideoWide = 1280;
				RecordCtrl.s_VideoHeight = 720;
				
				if(FAILED(GMI_RecordCtrl(&RecordCtrl)))
				{
					SYS_ERROR("###GMI_RecordCtrl fail, Result = 0x%lx\n", Result);
				}
				...
				if(FAILED(GMI_RecordParamConfig(&RecordParamConfig)))
				{
					...
					DEBUG_LOG(g_DefaultLogClient, e_DebugLogLevel_Exception, "#####GMI_RecordParamConfig, Result = 0x%lx\n", Result);
				}
			#endif
				break;
			case SYS_PTZCMD_RIGHTDOWN:
				...
				if(FAILED(GMI_RecordScheduleConfig(&RecordScheduleConfig)))
				{
					...
					DEBUG_LOG(g_DefaultLogClient, e_DebugLogLevel_Exception, "#####GMI_RecordScheduleConfig000, Result = 0x%lx\n", Result);
				}
			#endif
				break;
			}
